...
async function processImage(prompt: string) {
  try {
    const img2imgEndpoint = `${SD_API_URL}/sdapi/v1/img2img`;
    const extraImageEndpoint = `${SD_API_URL}/sdapi/v1/extra-single-image`;
    ...
    const response = await axios.post(img2imgEndpoint, payload);
    ...
    const pixelResponse = await axios.post(extraImageEndpoint, pixelPayload);
